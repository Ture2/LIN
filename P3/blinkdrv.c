			  ...
			  size_t len, loff_t *off)
{
	...
	char *kbuf = vmalloc(sizeof(char)*TAM_BUFF);

	/*Variables que manejan tokens*/
	int tok_cnt=0;
	char *tokens[NR_LEDS];

	/*Variables que manejan scanff*/
	int ledn=0;
	...
	/*Reservamos espacio a message, que se le pasara a usb_control_msg*/
	messages=kmalloc(NR_BYTES_BLINK_MSG*NR_LEDS,GFP_DMA);


	/* zero fill, memset, copia 0 a los primeros NR_BYTEs de message*/
	memset(messages,0,NR_BYTES_BLINK_MSG*NR_LEDS);


	if(kbuf[0] != '\n'){
		while(tok_cnt<NR_LEDS && (tokens[tok_cnt] = strsep(&kbuf, ",")) != NULL){
			retval = sscanf(tokens[tok_cnt],"%i:%i", &ledn, &color);


			if (retval < 2){
					printk(KERN_INFO "Argumento invalido\n");
					retval=-EINVAL;
					goto out_error;			
				}
				...
				if(ledn < 0 || ledn > 7){
					printk(KERN_INFO "Error: led number is too small or to big\n");
					retval=-EINVAL;
					goto out_error;	
				}

				if(color > 0xFFFFFF || color < 0x000000){
					printk(KERN_INFO "Error: invalid color\n");
					retval=-EINVAL;
					goto out_error;	
				}

			messages[3+6*ledn]=((color>>16) & 0xff);
		 	messages[4+6*ledn]=((color>>8) & 0xff);
		 	messages[5+6*ledn]=(color & 0xff);
	...
	for(i=0; i<NR_LEDS;i++){
		...
		/*if(message[2+6*i] == 0){
			messages[3+6*ledn]=((LED_OFF>>16) & 0xff);
	 		messages[4+6*ledn]=((LED_OFF>>8) & 0xff);
	 		messages[5+6*ledn]=(LED_OFF & 0xff);
		}*/
		messages[0+6*i]='\x05';
		messages[1+6*i]=0x00;
		messages[2+6*i] = i;
	 	...
	 	retval=usb_control_msg(dev->udev,	
			 ...
			 0x5,	/* wValue */
			 0, 	/* wIndex=Endpoint # */
			 (messages+6*i),	/* Pointer to the message */
